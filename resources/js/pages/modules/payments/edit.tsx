...
/**
 * Page de modification d'un mode de paiement
 */
const PaymentsEditPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // TODO: Implémenter l'appel API pour modifier le mode de paiement
      console.log('Updating payment method:', { id, ...formData });
      ...
      setErrors(error.response?.data?.errors || { general: 'Erreur lors de la modification' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer ce mode de paiement ? Cette action est irréversible.')) {
      return;
    }
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Modifier le Mode de Paiement</h1>
            <p className="text-muted-foreground">
              Modifier les détails de "{formData.name}"
            </p>
          </div>
        </div>
